<html>  
    <head>
		...
		var myID=0;	
			...
			var users = SecurityManager.GetAllUsers();
                var cmb1 = document.getElementById('cmbUser')
                for(var i=0;i<users.length;i++)
                {
                    var opt1 = document.createElement("option");
					//opt.style.width=150px;
                    opt1.setAttribute("value",users[i].login);
                    opt1.innerText = users[i].login;
	...
	{
	 var ary1= SecurityManager.GetAllUserRoles();
	var h="URManagement.php";
	var f=.1;
...
myTable+="<th style='width: 100px; color: black; text-align: center; border-collapse: collapse;border:1px solid;height:50px;'>Delete</th></tr>";
  ...
  for (var i=0; i<ary1.length; i++) {
    ...
    myTable+="<td style='width: 100px; text-align: center;height:50px; border-collapse: collapse;border:1px solid;'>"+ary1[i].user+"</td>";
    myTable+="<td style='width: 200px; text-align: center;height:50px; border-collapse: collapse;border:1px solid;'>"+ary1[i].role+"</td>";
 myTable+="<td style='width: 100px; text-align: center;height:50px; border-collapse: collapse;border:1px solid;'><a  id="+ ary1[i].ID +" href='#' onclick='EditUR(id);' >Edit</a></td>";
myTable+="<td style='width: 100px; text-align: center;height:50px; border-collapse: collapse;border:1px solid;'><a id="+i+" href='#' onclick='DeleteUR(this);'>Delete</a></td></tr>"; 
  }  
   myTable+="</table>";
   ...
   document.getElementById('myDiv').innerHTML = myTable;
	...
	var usr = document.forms["userForm"]["Uuser"].value;
	...
	else
	{
		...
		obj.role=rol;
			...
			SecurityManager.SaveUserRole(obj,Success,ErrorFind);
			myBool=false;
...
num++;
 ...
 em=Number(em);
 //alert(em);
 var ary2=SecurityManager.GetAllUserRoles();
 for (var i =0; i < ary2.length; i++){
   if (ary2[i].ID ==em) {
	   
      SecurityManager.DeleteUserRole(ary2[i].ID,Success,ErrorFind);
      break;
   }
 }
	myBuildTable(); 
...
function EditUR(vem1)
{
	var obj=SecurityManager.GetUserRoleById(vem1);
 var roles = SecurityManager.GetAllRoles();
                var cmb = document.getElementById('cmbRole');
				cmb.innerHTML = '';
                for(var i=0;i<roles.length;i++)
			...
			var users = SecurityManager.GetAllUsers();
                ...
                for(var i=0;i<users.length;i++)
                {
					...
					//opt.style.width=150px;
                    opt1.setAttribute("value",users[i].login);
					if (obj.user == users[i].login) {opt1.setAttribute("selected", true);}
                    opt1.innerText = users[i].login;
				...
				myID=vem1;
				myBool=true;
				//SecurityManager.DeleteUserRole(obj.ID,Success,ErrorFind);
}
        </script>
...
.div1 {
 margin-left:100px;margin-top:150px; border: 1px solid black; height:360px;width:400px; text-align:left;background-color:#404040;
...
<form name="userForm" method="get" >
	...
	<select name="Uuser" id="cmbUser" style="width:300px;height:30px;">
