<html>  
	...
	if(confirm("Are you Sure to delete Role?")){
	   ...
	   settings.type="GET";
				...
				settings.data={'val':id , 'act':"delRole"};
...
$(".edit").click(function(){
       ...
       var id=$(this).closest("tr").find(":first").text();
			...
			settings.type="GET";
				...
				settings.data={'val':id , 'act':"getRole"};
				settings.success=function(obj){
			 $("#Urole").val(obj[0]["name"]);
...
$("#submit").click(function(){
	   ...
	   if(myBool==true) //edit role
	   {
				...
				settings.dataType="json";
				settings.url="Api.php";
				settings.data={'id':gid,'rol':rol , 'act':"validateRI"};
				settings.success=function(obj){
					console.log(obj);
				if(obj=="exist")
				{
	   ...
	   else 
	   {
				...
				settings.dataType="json";
				settings.url="Api.php";
				settings.data={'rol':rol ,'act':"validateR"};
				settings.success=function(obj){
					console.log(obj);
				if(obj=="exist")
				{
...
function editRole(){
			...
			var des=$("#Udes").val();
				...
				settings.dataType="json";
				settings.url="Api.php";
				settings.data={'id':gid,'rol':rol ,'des':des,'act':"editRole"};
				settings.success=function(obj){
...
function insertRole()
	{
			...
			var des=$("#Udes").val();
				...
				settings.dataType="json";
				settings.url="Api.php";
				settings.data={'id':gid,'rol':rol ,'des':des , 'act':"addRole"};
				settings.success=function(obj){
...
myTable+="<th style='width: 100px; color: black; text-align: center; border-collapse: collapse;border:1px solid;height:50px;'>Delete</th></tr>";
  ...
  for (var i=0; i<ary1.length; i++) {
    myTable+="<tr><td style='width: 100px; text-align: center;height:50px; border-collapse: collapse;border:1px solid;'>"+ ary1[i].roleid +"</td>";
...
<form name="userForm" method="get" >
	...
	<input class="box" type="text" name="Urole" id="Urole" > 
	<br><br>
